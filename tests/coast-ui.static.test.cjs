...
test("HUD의 통신 기록 버튼은 부모의 pointer 차단을 명시적으로 해제한다", () => {
  const css = read("styles-20260903-volcano-20260905-upgrade.css");
...
test("통신 기록 모달은 접근 가능한 대화상자와 시간순 목록을 제공한다", () => {
  const html = read("index.html");
  const main = read("src/main-20260903-volcano-20260905-upgrade.js");
  const log = read("src/communication-log-20260829-coast-20260905-upgrade.js");
