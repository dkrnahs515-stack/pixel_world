...
test("coast and volcano browser checkpoints use only the deployed v8 save", () => {
  for (const source of [coastSmoke, volcanoSmoke]) {
    assert.doesNotMatch(source, /pixel-world\.progress\.v[1-7]:/);
    assert.match(source, /pixel-world\.progress\.v8:/);
  }
  ...
  assert.match(volcanoSmoke, /game-20260910-sanctuary\.js/);
  assert.match(volcanoSmoke, /main-20260910-sanctuary\.js/);
});
