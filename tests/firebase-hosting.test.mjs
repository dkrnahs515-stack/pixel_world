import test from "node:test";
import assert from "node:assert/strict";
import { existsSync, readFileSync } from "node:fs";

const firebase = JSON.parse(readFileSync(new URL("../firebase.json", import.meta.url), "utf8"));
const index = readFileSync(new URL("../index.html", import.meta.url), "utf8");

test("Firebase의 고정 URL JavaScript와 CSS는 새 배포마다 재검증한다", () => {
  const assetRule = firebase.hosting.headers.find(rule => rule.source.includes("js|css"));
  const cacheControl = assetRule?.headers.find(header => header.key.toLowerCase() === "cache-control")?.value;

  assert.match(cacheControl ?? "", /(?:^|,\s*)no-cache(?:,|$)/);
  assert.doesNotMatch(cacheControl ?? "", /max-age=[1-9]\d*/);
});

test("진입 HTML은 CSS와 JavaScript의 실제 파일명으로 기존 배포 캐시를 우회한다", () => {
  const stylesheetPath = index.match(/href="(\.\/styles-([^"]+?)\.css)"/)?.[1];
  const stylesheetVersion = index.match(/href="\.\/styles-([^"]+?)\.css"/)?.[1];
  const modulePath = index.match(/src="(\.\/src\/main-([^"]+?)\.js)"/)?.[1];
  const moduleVersion = index.match(/src="\.\/src\/main-([^"]+?)\.js"/)?.[1];

  assert.ok(stylesheetVersion);
  assert.equal(moduleVersion, stylesheetVersion);
  assert.ok(stylesheetPath);
  assert.ok(existsSync(new URL(`..\/${stylesheetPath}`, import.meta.url)));
  assert.ok(modulePath);
  assert.ok(existsSync(new URL(`..\/${modulePath}`, import.meta.url)));
  assert.doesNotMatch(index, /(?:href|src)="[^"]+\?v=/);

  const entry = readFileSync(new URL(`..\/${modulePath}`, import.meta.url), "utf8");
  const gameModulePath = entry.match(/from "(\.\/game-([^"]+?)\.js)"/)?.[1];
  const gameModuleVersion = entry.match(/from "\.\/game-([^"]+?)\.js"/)?.[1];

  assert.equal(gameModuleVersion, stylesheetVersion);
  assert.ok(gameModulePath);
  assert.ok(existsSync(new URL(`..\/src\/${gameModulePath}`, import.meta.url)));
});

test("성역 release는 변경된 전이 모듈과 CSS를 물리 파일 체인으로 연결한다", () => {
  assert.match(index, /href="\.\/styles-20260910-sanctuary\.css"/);
  assert.match(index, /src="\.\/src\/main-20260910-sanctuary\.js"/);
  assert.doesNotMatch(index, /main-20260903-volcano-20260905-upgrade\.js/);
  for (const relativePath of [
    "../styles-20260910-sanctuary.css",
    "../src/main-20260910-sanctuary.js",
    "../src/game-20260910-sanctuary.js",
    "../src/coop-boss-controller-20260910-sanctuary.js",
    "../src/network-20260910-sanctuary.js",
    "../src/coop-boss-network-20260910-sanctuary.js",
    "../src/progress-storage-20260910-sanctuary.js",
    "../src/first-journey-controller-20260910-sanctuary.js",
    "../src/aren-dialogue-20260910-sanctuary.js",
  ]) {
    assert.ok(existsSync(new URL(relativePath, import.meta.url)), `${relativePath} must exist`);
  }

  const entry = readFileSync(new URL("../src/main-20260910-sanctuary.js", import.meta.url), "utf8");
  const game = readFileSync(new URL("../src/game-20260910-sanctuary.js", import.meta.url), "utf8");
  const network = readFileSync(new URL("../src/network-20260910-sanctuary.js", import.meta.url), "utf8");
  assert.match(entry, /from "\.\/game-20260910-sanctuary\.js"/);
  assert.match(entry, /from "\.\/first-journey-controller-20260910-sanctuary\.js"/);
  assert.match(game, /from "\.\/coop-boss-controller-20260910-sanctuary\.js"/);
  assert.match(game, /from "\.\/network-20260910-sanctuary\.js"/);
  assert.match(game, /from "\.\/aren-dialogue-20260910-sanctuary\.js"/);
  assert.match(game, /from "\.\/quest-guidance-20260910-sanctuary\.js"/);
  assert.match(network, /from "\.\/coop-boss-network-20260910-sanctuary\.js"/);
});

test("Firebase Hosting 배포 대상에서 개발 문서·테스트·worktree를 제외한다", () => {
  const ignored = firebase.hosting.ignore;
  for (const pattern of ["README.md", "docs/**", "tests/**", ".worktrees/**", ".github/**"]) {
    assert.ok(ignored.includes(pattern), `${pattern} must be ignored`);
  }
});
