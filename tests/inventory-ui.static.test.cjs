...
test("장비 영역은 시작 검과 여섯 상위 무기를 제공하고 카드에서 직접 장착한다", () => {
  assert.equal((html.match(/data-inventory-weapon="[^"]+"/g) || []).length, 7);
  assert.equal((html.match(/data-equip-weapon="[^"]+"/g) || []).length, 7);
  assert.match(html, /data-inventory-weapon="starter-sword"[^>]*>[\s\S]*?시작 검[\s\S]*?장착 중/);
  for (const name of ["카타나", "강화 카타나", "상급 카타나", "정예 카타나", "명검", "강화 명검"]) {
...
test("인벤토리 DOM 요소는 게임에 연결된다", () => {
  ...
  assert.ok(main.includes('document.querySelectorAll("[data-inventory-weapon]")'));
  assert.ok(main.includes('document.querySelectorAll("[data-equip-weapon]")'));
});
