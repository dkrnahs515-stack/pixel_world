...
test("대화 중 Enter는 주요 행동에 남고 Escape만 대화를 닫는다", async () => {
  const { dialogueKeyAction } = await import("../src/game-20260902-lease.js");
...
test("대화 포커스는 주요 행동과 닫기 버튼 안에서 순환한다", async () => {
  const { nextDialogueFocus } = await import("../src/game-20260902-lease.js");
  const actionButton = {};
  const closeButton = {};
...
test("읽기가 차단된 브라우저 저장소는 사용 불가로 구분한다", async () => {
  const { readableProgressStorage } = await import("../src/game-20260902-lease.js");
