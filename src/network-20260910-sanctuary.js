import { FIREBASE_CONFIG, GAME_CONFIG as C, ROOM_ID } from "./config-20260905-upgrade.js";
import { createFirebaseChatAdapter, createOfflineChatAdapter } from "./chat-network-20260903-volcano-20260905-upgrade.js";
import { filterPlayersForMap, serializePlayerState } from "./network-state-20260910-sanctuary.js";
import { createPublishPolicyState, nextPublishDecision } from "./network-publish-policy-20260905-upgrade.js";
import { claimRoomSlot } from "./room-capacity-20260905-upgrade.js";
import { createCoopBossNetwork } from "./coop-boss-network-20260910-sanctuary.js";
...
export async function createNetworkAdapter(callbacks = {}, dependencies = {}) {
  ...
  try {
    ...
    const unsubscribeOwnPlayer = dbModule.onValue(playerRef, snapshot => {
      ...
      ownJoinedAt = Number.isFinite(value?.joinedAt) ? value.joinedAt : Number.POSITIVE_INFINITY;
    ...
    const publish = (state, mapId) => {
      if (stopped) return;
      const serialized = serializePlayerState(state, mapId);
      if (!serialized) return;
      if (serialized.mapId !== activeMapId) {
        activeMapId = serialized.mapId;
        subscribePlayersForMap(activeMapId);
      }
      ...
      const update = { ...serialized, updatedAt: dbModule.serverTimestamp() };
      if (includeJoinedAt) {
    ...
    return {
      ...
      stop: async () => {
        ...
        disconnectTimer = null;
        await coopBoss.stop();
        try { await roomSlot.release(); } catch (error) { console.warn("온라인 슬롯 정리 실패", error); }
        ...
        try { dbModule.goOffline?.(db); } catch (error) { console.warn("Firebase 세션 종료 실패", error); }
      },
    };
  } catch (error) {
    try { await roomSlot?.release?.(); } catch { /* keep original connection error */ }
    try { activeFirebaseSession?.dbModule?.goOffline?.(activeFirebaseSession.db); } catch { /* keep original connection error */ }
