...
export function drawProjectile(ctx, projectile, options = {}) {
  const kinds = new Set(["arrow", "piercing-arrow", "magic-bolt", "explosive-bolt", "ice-bolt", "spread-arrow"]);
  ...
  if (projectile.kind === "arrow" || projectile.kind === "piercing-arrow" || projectile.kind === "spread-arrow") {
...
export function drawExplosionEffect(ctx, effect, options = {}) {
  ...
  if (effect.delivery === "area") {
    ctx.fillStyle = "#fde68a";
    for (let i = 0; i < 12; i += 1) {
      const angle = i * 2.4, distance = effect.radius * Math.sqrt((i + 1) / 12) * 0.8;
      const arrowX = x + Math.cos(angle) * distance, arrowY = y + Math.sin(angle) * distance - remaining * 35;
      ctx.fillRect(arrowX, arrowY - 20, 2, 20); ctx.fillRect(arrowX - 2, arrowY - 3, 6, 3);
    }
    ctx.restore(); return true;
  }
  ctx.fillStyle = effect.delivery === "meteor" ? "#f97316" : "#f0abfc";
