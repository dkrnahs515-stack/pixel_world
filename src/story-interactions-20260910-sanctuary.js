...
function transitionForSanctuaryInteraction(progress, interaction) {
  switch (interaction.type) {
    case "sanctuary-resonance":
      return activateSanctuaryResonanceNode(progress, interaction.id);
    case "sanctuary-archive":
      return restoreSanctuaryArchive(progress, interaction.id);
    case "sanctuary-origin-record":
      return collectOriginRecord(progress, interaction.id);
    default:
      return { progress, effects: [] };
  }
}
...
function sanctuaryArchiveSequenceReady(interaction, progress) {
  if (interaction.type !== "sanctuary-archive") return true;
  const order = Number.isInteger(interaction.order) ? interaction.order : 0;
  const earlierIds = SANCTUARY_STORY_INTERACTIONS
    .filter(value => value.type === "sanctuary-archive" && value.order < order)
    .sort((a, b) => a.order - b.order)
    .map(value => value.id);
  return earlierIds.every(id => progress.chapters.sanctuary.restoredArchiveIds.includes(id));
}
...
export function isStoryInteractionEligible(interaction, worldProgress) {
  ...
  if (interaction?.chapterId === "sanctuary") {
    if (!sanctuaryArchiveSequenceReady(interaction, progress)) return false;
    return changed(progress, transitionForSanctuaryInteraction(progress, interaction).progress);
  }
  return isCoastStoryInteractionEligible(interaction, progress);
}
...
function resolveSanctuaryInteraction(progress, interaction) {
  const initial = normalizeWorldProgress(progress);
  if (!isStoryInteractionEligible(interaction, initial)) {
    return result(initial, interaction.id, "unavailable");
  }
  const resolved = transitionForSanctuaryInteraction(initial, interaction);
  return result(
    resolved.progress,
    interaction.id,
    changed(initial, resolved.progress) ? "completed" : "unavailable",
...
export function resolveStoryInteraction(progress, interactionId, response) {
  ...
  if (interaction?.chapterId === "sanctuary") {
    return resolveSanctuaryInteraction(progress, interaction);
  }
  const initial = normalizeWorldProgress(progress);
