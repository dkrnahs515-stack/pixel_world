...
function isWorldPositionBlocked(mapId,x,y,radius) {
  const world=getWorldDefinition(mapId);
  return x-radius<0 || y-radius<0 || x+radius>world.width || y+radius>world.height
    || world.obstacles.some(rect=>rect.solid!==false && pointInRect(x,y,rect,radius));
}
...
export function isBossInSkillGeometry(resource, attack, hitIndex, encounter, definition) {
  ...
  const radius = definition.radius || getEnemyDefinition(definition.enemyKind)?.radius || 24;
