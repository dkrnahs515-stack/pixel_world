...
import {
  acquireAuthority,
  claimReward as claimRewardState,
  createBossEncounter,
  renewAuthority as renewAuthorityState,
} from "./coop-boss-state-20260910-sanctuary.js";
import {
...
export function createCoopBossNetwork({
  ...
  const isFinalBoss = () => definition()?.bossClass === "final";
  ...
  const api = {
    ...
    async ensureEncounter({ partySize = 1, reconciledEncounterId = null } = {}) {
      ...
      if (!bossDefinition || partySize < 1) return null;
      ...
      const transaction = await dbModule.runTransaction(stateRef, current => {
        ...
        return bossDefinition.bossClass === "final"
          ? createOriginEncounter(bossDefinition, options)
          : createBossEncounter(bossDefinition, options);
      });
    ...
    async tryAcquireAuthority() {
      if (!definition()) return { ok: false, reason: "no_boss" };
      let outcome = { ok: false, reason: "transaction_aborted" };
      const transaction = await dbModule.runTransaction(pathRef("state"), current => {
        outcome = isFinalBoss()
          ? acquireOriginAuthority(current, { uid, now: now() })
          : acquireAuthority(current, { uid, now: now() });
    ...
    async renewAuthority(epoch = authorityEpoch) {
      ...
      const transaction = await dbModule.runTransaction(pathRef("state"), current => {
        outcome = isFinalBoss()
          ? renewOriginAuthority(current, { uid, authorityEpoch: epoch, now: now() })
          : renewAuthorityState(current, { uid, authorityEpoch: epoch, now: now() });
        return outcome.ok ? outcome.encounter : undefined;
      });
    ...
    async sendAttack(request) {
      if (!definition() || !Number.isInteger(request?.sequence) || request.sequence < 1) {
    ...
    async cleanupExpired() {
      if (!definition() || typeof dbModule.get !== "function") return { ok: true, removed: 0 };
      const timestamp = now();
