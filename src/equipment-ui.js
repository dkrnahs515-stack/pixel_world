...
import { normalizeClassEquipment } from "./equipment-state.js";
import { getWeaponsForClass } from "./weapon-data.js";
import { drawWeaponPreview } from "./weapon-rendering.js";
...
export function equipmentUiModel({ classId, level, gold, equipment }) {
  ...
  const paidWeapons = weapons.filter(weapon => weapon.price !== null);
  return {
    classId: normalizedClassId,
    equippedWeaponId: normalizedEquipment.equippedWeaponId,
    buyItems: paidWeapons.map(weapon => buyItem(weapon, level, gold, owned)),
    sellItems: paidWeapons
      .filter(weapon => owned.has(weapon.id))
      .map(weapon => ({
        weapon,
...
function masterworkClass(weapon) {
  return weapon.tier >= 6 ? " masterwork" : "";
}
...
function renderBuyItem(documentRef, item) {
  ...
  const button = element(documentRef, "button", {
    ...
    dataset: { buyWeapon: item.weapon.id },
  });
  button.type = "button";
...
function renderSellItem(documentRef, item) {
  ...
  const button = element(documentRef, "button", {
    text: item.buttonLabel,
    dataset: { sellWeapon: item.weapon.id },
  });
  button.type = "button";
...
function renderInventoryItem(documentRef, item) {
  ...
  copy.append(
    ...
    element(documentRef, "small", {
      text: `Lv.${item.weapon.requiredLevel} · ${item.statsLabel}`,
    }),
  );
  const button = element(documentRef, "button", {
    text: item.buttonLabel,
    dataset: { equipWeapon: item.weapon.id },
  });
  button.type = "button";
  button.disabled = item.disabled;
  card.append(copy, button);
