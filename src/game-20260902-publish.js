...
import { layoutChatBubble, worldToScreen } from "./chat-bubble-layout-20260829-coast.js";
import { ChatController } from "./chat-controller-20260829-coast.js";
import { latestBubblesByUid } from "./chat-state-20260829-coast.js";
...
import { createNetworkAdapter, createOfflineNetworkAdapter } from "./network-20260902-publish.js";
...
import { getRegionForMap } from "./region-data-20260829-coast.js";
import {
  COAST_STORY_INTERACTIONS,
  getCoastChapterObjective,
  getCollectedCoastRecords,
} from "./coast-story-data-20260829-coast.js";
import { coastActorDialogueModel, coastStoryDialogueModel } from "./story-dialogue-20260829-coast.js";
import {
  findNearbyStoryInteraction,
  resolveStoryInteraction,
  storyInteractionPrompt,
} from "./story-interactions-20260829-coast.js";
import { getNpcsForWorld } from "./npc-data-20260829-coast.js";
...
import { loadProgressWithStatus, saveProgress } from "./progress-storage-20260829-coast.js";
import {
...
import {
  ...
  recordAdventureKill,
} from "./quest-state-20260829-coast.js";
import { arenDialogueModel } from "./aren-dialogue-20260829-coast.js";
import { getWorldDefinition, normalizeWorldId } from "./world-data-20260829-coast.js";
import {
  createWorldLayer,
...
export class PixelRPG {
  ...
  processBossReward(event, recipientUid) {
    ...
    } else if (region?.id === "coast") {
      worldTransition = recordChapterBossDefeat(reward.progress.worldProgress, "coast");
    }
  ...
  openStoryInteraction(interaction = this.nearbyStoryInteraction) {
    ...
    this.dialogue.open(coastStoryDialogueModel(interaction, this.progress.worldProgress));
  ...
  openNpcDialogue(npc = findNearbyNpc(this.npcs, this.player)) {
    ...
    const model = npc.role === "quest" && npc.id === "aren"
      ...
      : npc.actorId
        ? coastActorDialogueModel(npc.actorId, this.progress.worldProgress)
        : null;
  ...
  applyStoryInteraction(interactionId, response) {
    ...
    if (result.outcome === "acknowledged") return true;
  ...
  currentChapterObjective() {
    return getCoastChapterObjective(this.progress?.worldProgress);
  }
  ...
  updateNpcPrompt() {
    ...
    this.nearbyStoryInteraction = eligible
      ? findNearbyStoryInteraction(COAST_STORY_INTERACTIONS, { ...this.player, mapId: this.mapId }, this.progress?.worldProgress)
      : null;
