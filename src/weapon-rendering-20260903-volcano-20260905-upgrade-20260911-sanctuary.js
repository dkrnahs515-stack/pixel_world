...
const resolveDirectionAngle = direction => Object.hasOwn(DIRECTION_ANGLES, direction)
...
export function drawBow(ctx, { visual }) {
  const halfLength = Math.floor(visual.limbLength / 2);
  ctx.fillStyle = visual.woodColor;
  ctx.fillRect(8, -10, halfLength, visual.limbWidth);
  ctx.fillRect(8, 10 - visual.limbWidth, halfLength, visual.limbWidth);
  ctx.fillRect(8 + halfLength - visual.limbWidth, -8, visual.limbWidth, 16);
...
export function drawWeapon(ctx, {
  direction = "down",
  attackState = null,
  classId = null,
  ...
  ctx.save();
  ctx.rotate(resolveDirectionAngle(direction) + swing);
  if (weapon.weaponType === "bow") drawBow(ctx, { visual: weapon.visual });
  else if (weapon.weaponType === "staff") drawStaff(ctx, { visual: weapon.visual });
  else drawSword(ctx, { visual: weapon.visual });
  ctx.restore();
}

export function drawScabbard(ctx, { classId = null, direction = "down", weaponId = STARTER_WEAPON_ID } = {}) {
  const weapon = resolveRenderedWeapon(weaponId, classId);
  ...
  ctx.save();
  ctx.rotate(resolveDirectionAngle(direction) + Math.PI * 0.72);
