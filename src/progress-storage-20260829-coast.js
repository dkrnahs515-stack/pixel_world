...
import { normalizeWorldProgress } from "./chapter-progress-20260829-coast.js";

const STORAGE_VERSION = 6;
const STORAGE_PREFIX = "pixel-world.progress.v6:";
...
export function v5ProgressStorageKey(nickname) {
  return `${V5_STORAGE_PREFIX}${encodeURIComponent(normalizeNickname(nickname))}`;
}
...
export function loadProgressWithStatus(storage, nickname) {
  try {
    const v6 = parseStoredValue(storage?.getItem(progressStorageKey(nickname)));
    if (v6?.version === STORAGE_VERSION && isValidProgress(v6)) {
      return {
        progress: toProgress(v6),
