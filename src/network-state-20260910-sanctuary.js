...
import { WEAPON_ORDER as LEGACY_SWORD_WEAPON_IDS } from "./weapon-data-20260905-upgrade.js";
import { resolveWeaponDefinition } from "./weapon-data-20260903-volcano-20260905-upgrade.js";

function skillResources(player, classId) {
  const maxMp = statsForLevel(
    Number.isInteger(player.level) && player.level > 0 ? Math.min(10000, player.level) : 1,
    classId,
  ).maxMp;
...
export function serializePlayerState(player, mapId) {
  ...
  const level = Number.isInteger(player.level) && player.level > 0 && player.level <= 10000
    ? player.level
    : 1;
  return {
    ...
    level,
    mp: Number.isFinite(player.mp)
      ? Math.max(0, Math.min(player.mp, statsForLevel(level, classId).maxMp))
      : 0,
...
export function filterPlayersForMap(rawPlayers, ownUid, activeMapId) {
  ...
  const world = getWorldDefinition(activeMapId);
  const players = new Map();

  Object.entries(rawPlayers || {}).forEach(([uid, raw]) => {
    if (uid === ownUid || raw?.mapId !== activeMapId) return;
    ...
    const level = Number.isInteger(raw.level) && raw.level > 0 && raw.level <= 10000 ? raw.level : 1;
    players.set(uid, {
      ...raw,
      ...
      pencilWeapon: false,
      level,
      mp: Number.isFinite(raw.mp)
        ? Math.max(0, Math.min(raw.mp, statsForLevel(level, classId).maxMp))
        : 0,
